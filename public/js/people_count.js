...
function getTableData(){
  ...
  getDataPromise = new Promise((resolve, reject) => {
    $.get('public/overallData/peopleCount.csv', function(data){
      resolve(data)
    });
  }).then(
    function (result) {
      ...
      return new Promise((resolve, reject) => {
        $.get('public/subjectsData/' + subject + '/' + subject + '-peopleCount.csv', function (data) {
          resolve(data)
        });
      })
    }
...
function drawChart(dataTable) {
  ...
  var fontsize = 13
  var chartoptions = {
    title : 'How many individuals were you close to?',
    'height' : 650,
    //curveType: 'function',
    'intervals': { 
      'style' : 'bars' ,
