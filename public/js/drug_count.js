...
function getTableData(){
  ...
  getDataPromise = new Promise((resolve, reject) => {
    $.get('public/overallData/drugCount.csv', function(data){
      resolve(data)
    });
  }).then(
    function (result) {
      ...
      return new Promise((resolve, reject) => {
        $.get('public/subjectsData/' + subject + '/' + subject + '-drugs.csv', function (data) {
          resolve(data)
        });
      })
    }
...
function drawChart(dataTable) {
  ...
  var fontsize = 13
  var chartoptions = {
    title : 'How many [drug types] were you exposed to?',
    subtitle: 'How many [drug types] were you exposed to?',
    'height' : 650,
    //curveType: 'function',
    'intervals': { 
      'style' : 'bars' ,
